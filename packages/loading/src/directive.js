...
loadingDirective.install = Vue => {
  ...
  Vue.directive('loading', {
    bind: function(el, binding, vnode) {
      // 获取相关参数如text，background，spinner，customClass
      const textExr = el.getAttribute('wlkj-loading-text');
      const spinnerExr = el.getAttribute('wlkj-loading-spinner');
      const backgroundExr = el.getAttribute('wlkj-loading-background');
      const customClassExr = el.getAttribute('wlkj-loading-custom-class');
    ...
    update: function(el, binding) {
      el.instance.setText(el.getAttribute('wl-loading-text'));
      if (binding.oldValue !== binding.value) {
        toggleLoading(el, binding);
      }
    },
    unbind: function(el, binding) {
      // 当指令绑定的元素解绑时或者销毁时，只触发一次，如绑定元素被其它方式移除，通过指令生成的dom节点也需要销毁
      if (el.domInserted) {
        el.mask &&
        el.mask.parentNode &&
        el.mask.parentNode.removeChild(el.mask);
        toggleLoading(el, {vaule: false, modifiers: binding.modifiers});
      }
