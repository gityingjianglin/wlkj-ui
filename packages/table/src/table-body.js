import { arrayFindIndex } from 'wlkj-ui/src/utils/util';
import { getCell, getColumnByCell, getRowIdentity } from './util';
import { getStyle, hasClass, removeClass, addClass } from 'wlkj-ui/src/utils/dom';
import WlCheckbox from 'wlkj-ui/packages/checkbox';
import WlTooltip from 'wlkj-ui/packages/tooltip';
...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <wl-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent}></wl-tooltip>
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.wl-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['wl-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('wl-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'wl-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('wl-table__row--level-' + treeRowData.level);
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class="wl-table__cell wl-table__expanded-cell">
