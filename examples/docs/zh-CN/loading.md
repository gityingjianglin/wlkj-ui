...
<template>
  <ul v-loading="loading">
    <li v-for="item in listData">{{item.name}} - {{item.address}}</li>
  </ul>
</template>
...
<template>
  <ul 
    v-loading="loading"
    wlkj-loading-text="拼命加载中"
    wlkj-loading-spinner="wl-icon-ClockCircleFilled"
    wlkj-loading-background="rgba(0, 0, 0, 0.8)"
  >
    <li v-for="item in listData">{{item.name}} - {{item.address}}</li>
  </ul>
</template>
    ...
    methods: {
      ...
      openFullScreen2() {
        const loading = this.$loading({
          lock: true,
          text: 'Loading',
          spinner: 'wl-icon-ClockCircleFilled',
