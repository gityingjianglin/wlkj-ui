...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.wl-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.wl-switch__label--left').querySelector('span').textContent).to.equal('off');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'wl-icon-check',
      inactiveIconClass: 'wl-icon-close'
    });

    const icon = vm.$el.querySelector('.wl-switch__label--left').querySelector('i');
    expect(icon.classList.contains('wl-icon-close')).to.true;
  });
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <wl-switch disabled v-model="value"></wl-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.wl-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <wl-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></wl-switch>
        </div>
      `,
      data() {
        return {
  ...
  it('value is the single source of truth', done => {
    vm = createVue({
      template: `
        <div>
          <wl-switch :value="true"></wl-switch>
        </div>
      `
    }, true);
  ...
  it('sets checkbox value', done => {
    vm = createVue({
      template: `
        <div>
          <wl-switch v-model="value"></wl-switch>
        </div>
      `,
      data() {
        return {
