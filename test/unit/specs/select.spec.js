...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.className).to.equal('wl-select');
    expect(vm.$el.querySelector('.wl-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.wl-select-dropdown__item');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.wl-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value" @change="handleChange">
            <wl-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.wl-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value" @visible-change="handleVisibleChange">
            <wl-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
    ...
    // 测试用例vm也是一个vue组件，包括的第一个子元素组件就是wl-select
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.wl-input__icon.wl-icon-CloseCircleOutlined');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value" value-key="id">
            <wl-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.wl-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.wl-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
    ...
    expect(vm.$el.querySelector('.wl-input__icon').classList.contains('wl-icon-SearchOutline')).to.be.true;
  });

  it('custom wl-option template', () => {
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value">
    ...
    expect(vm.$el.querySelector('.wl-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.wl-select-group__wrap');
    const options = groups[1].querySelectorAll('.wl-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.wl-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.wl-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <wl-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('event:focus & blur', done => {
    vm = createVue({
      template: `
        <wl-select ref="select"></wl-select>
      `
    }, true);
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value" ref="select">
            <wl-option label="1" :value="1" />
          </wl-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.wl-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <wl-select v-model="value" @change="change" ref="select">
            <wl-option label="1" :value="1" />
            <wl-option label="2" :value="2" />
            <wl-option label="3" :value="3" />
          </wl-select>
        </div>
      `,
      data() {
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async() => {
    vm = createVue({
      template: `
        <div>
          <wl-select ref="select" v-model="value" filterable>
            <wl-option label="test" value="test" />
          </wl-select>
        </div>
      `,
      data() {
    ...
    expect(vm.$el.querySelector('.wl-input__inner').placeholder).to.be.equal('test');
  ...
  it('default value is null or undefined', async() => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.wl-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.wl-input__inner').value).to.equal('黄金糕');
  });
