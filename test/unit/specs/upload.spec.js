...
describe('ajax', () => {
  beforeEach(() => {
    // 使用sinon构造的虚拟的xhr对象屏蔽原始js的xhr对象，并且在使用完毕以后，通过restore()方法返回js原始的xhr对象
...
describe('Upload', () => {
  ...
  describe('ajax upload', () => {
    ...
    it('preview file', done => {
      ...
      handlers.onSuccess = (res, file, fileList) => {
        uploader.$nextTick(_ => {
          uploader.$el.querySelector('.wl-upload-list .is-success a').click();
        });
      };
    ...
    it('file remove', done => {
      ...
      handlers.onSuccess = (res, file, fileList) => {
        uploader.$el.querySelector('.wl-upload-list .wl-icon-CloseOutlined').click();
        uploader.$nextTick(_ => {
